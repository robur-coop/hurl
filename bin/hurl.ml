...
let print_address ppf (ipaddr, port) =
  let color_of_port = match port with
    | 80 -> `Blue | 443 -> `Green | _ -> `Yellow in
  Fmt.pf ppf "%a:%a\n%!" Fmt.(styled `Magenta Ipaddr.pp) ipaddr
    Fmt.(styled color_of_port int) port
...
type state = Begin | Hex of (unit, Hex.n) result Hex.command | Raw | Json | Ignore
...
let ready_for_a_redirection ~uri resp =
  ...
  if Status.is_redirection resp.status
  then match Headers.get resp.headers "location" with
    | Some location ->
...
let guess_how_to_print ?(hex= false) resp =
  if hex then `Hex
  else match recognize_mime_type resp, content_encoding resp with
  | Some (`Text, _), None -> `Raw
  | _ -> `Hex

let run ?(hex = true) (hxd_cfg, stdout) tls_config http_version ~follow_redirect meth uri request_items output =
  ...
  in
  let config = match http_version with
  ...
  let rec fn ((ipaddr, port), _epoch as meta) resp acc str =
    ...
    | _, Begin when ready_for_a_redirection ~uri resp && follow_redirect ->
        ...
        print_response Fmt.stdout resp; Fmt.pr "\n%!"; Ignore
    | None, Begin -> print_response Fmt.stdout resp; Begin
    | Some str, Begin ->
        print_address Fmt.stdout (ipaddr, port);
        print_response Fmt.stdout resp;
        Fmt.pr "\n%!";
        begin match guess_how_to_print ~hex resp with
        | `Raw -> fn meta resp Raw (Some str)
        | `Hex -> fn meta resp (Hex (Hex.generate hxd_cfg ppf)) (Some str) end
  ...
  Httpcats.request ?config ?tls_config ~follow_redirect ?meth ~headers ?body ~f:fn ~uri Begin
  |> function
  | Ok (resp, Begin) ->
      print_response Fmt.stdout resp
  | Ok (_resp, _) -> ()
...
let run (_quiet, stdout) hxd_cfg (tls_cfg, http_version) follow_redirect hex meth uri request_items output =
  Miou_unix.run @@ fun () ->
  let rng = Mirage_crypto_rng_miou_unix.(initialize (module Pfortuna)) in
  let ret =
    try
      run ~hex (hxd_cfg, stdout) tls_cfg http_version ~follow_redirect meth uri request_items output;
      `Ok 0
