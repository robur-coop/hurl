module Uri = struct
  ...
  let pct_decode str =
    ...
    let rec scan start cur =
      if cur >= len then Buffer.add_substring buf str start (cur - start)
        ...
        else match int_of_hex_char str.[cur] with
          | exception _ ->
              Buffer.add_char buf '%';
              scan cur cur
          | high ->
              ...
              if cur >= len then begin
                ...
                let start_at = match int_of_hex_char str.[cur] with
                  | low ->
  ...
  let query_decode str =
    ...
    let split str = match String.index_opt str '=' with
      | None -> [ str ]
      | Some pos ->
  ...
  let path_decode str =
    ...
    | None -> match String.index_opt str '#' with
      | Some pos -> String.sub str 0 pos
      | None -> str

  let decode str = match String.index_opt str '?' with
    | None -> None
    | Some pos ->
      let query = String.sub str (pos + 1) (String.length str - pos - 1) in
      let query = query_decode query in
      let query = List.map (fun (k, v) -> pct_decode k, List.map pct_decode v) query in
      Some query
end
...
let json_of_meta ~host:default_host meta request =
  ...
  let origin, scheme = match meta with
    | `Tcp socket -> 
    ...
    | `Tls socket ->
      ...
      let socket = Miou_unix.to_file_descr socket in
      let sockaddr = Unix.getpeername socket in
      `String (Fmt.to_to_string pp_origin sockaddr), "https" in
  let host = H1.Headers.get request.H1.Request.headers "Host" in
  let host = Option.value ~default:default_host host in
  let url = `String (Fmt.str "%s://%s%s" scheme host request.H1.Request.target) in
  let args = match Uri.decode request.H1.Request.target with
    | None -> `O []
    | Some queries ->
      let queries = List.map (fun (k, v) -> k, `String (String.concat "," v)) queries in
      `O queries in
  `O [ "args", args
     ; "headers", headers
     ; "origin", origin
...
let headers_http_1_1 ?(content_type= "application/json") ?content_length () =
  ...
  Option.fold ~none:hdrs ~some:(fun content_length ->
    let value = string_of_int content_length in
    let hdrs = H1.Headers.add hdrs "Content-Length" value in
    H1.Headers.add hdrs "Connection" "close") content_length

let handler_http_1_1 ~host meta reqd =
  let request = H1.Reqd.request reqd in
  match request.H1.Request.meth, Uri.path_decode request.H1.Request.target with
  | `GET, "/get" ->
  ...
  | `GET, "/gzip" ->
    ...
    let content_length = String.length body in
    let headers = headers_http_1_1 ~content_length () in
    let headers = H1.Headers.add headers "Content-Encoding" content_encoding in
    let response = H1.Response.create ~headers `OK in
    H1.Reqd.respond_with_string reqd response body
  | `GET, "/deflate" ->
    let json = json_of_meta ~host meta request in
...
|text} in
      ...
      H1.Reqd.respond_with_string reqd response body
  | _ ->
    let body = {html|<!doctype html><title>404 Not found</title><h1>Not found</h1>|html} in
    let content_length = String.length body in
    let headers = headers_http_1_1 ~content_type:"text/html" ~content_length () in
...
let on =
  let doc = "The address where we mount the HTTP server (including the port)." in
  ...
  let[@warning "-8"] Ok default = Ipaddr.with_port_of_string ~default:8000 "127.0.0.1:8000" in
...
let pid =
  ...
  Arg.(value & opt (some non_existing_file) None & info [ "p"; "pid" ] ~doc ~docv:"FILE")
...
let cmd =
  let doc = "A simple server to test multiple ways to communicate with an HTTP service." in
  let man = [] in
